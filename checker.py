...
async def main_checker():
    ...
    for index, address in enumerate(addresses):
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            if address.startswith('bc1p'):
                ...
                if btc_balance.confirmed:
                    magic_unisat = get_magic_unisat(unisat=remove_duplicates(account=unisat_account),
                                                    magic=magic_tokens)
                    ...
                    if magic_unisat:
                        ...
                        logger.warning(f"empty: no any collection or token.")
                    logger.info(int(len(addresses[0]) * 2) * '-')
                else:
                    fully_empty_addresses.append((index, address))
            else:
                ...
                if btc_balance.confirmed:
                    btc_total += btc_balance.confirmed
                    logger.success(f"#{index + 1} | {address} | not a taproot address | "
                                   f"{btc_balance.confirmed / btc_denom:.{btc_round}f} $BTC.")
    ...
    if fully_empty_addresses and show_empty_wallets:
        logger.warning(f"fully_empty_wallets:")
        for addr in fully_empty_addresses:
            logger.info(f"#{addr[0] + 1} | {addr[1]}.")
